...
const updateTypechecking = () => {
  const val = editor.getValue();
  const all = before + val + after;
  // Make a request for a user with a given ID
  axios
    .post("http://localhost:3000/typecheck", { code: all })
    ...
    .catch(function(error) {
      ...
      const niceErrors = error.response.data.join("\n");
      errorBox.innerText = "Oh no!\n" + niceErrors;
    });
};
